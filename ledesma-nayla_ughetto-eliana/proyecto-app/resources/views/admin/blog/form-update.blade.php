...
?>
@section('title' , 'Editar post - EPUB Depository')
@extends('layouts.main')
@section('main')
<h1>Editar Post {{$posts->titulo}}</h1>
<form action="{{route('post-process-edit' , ['id' => $posts->post_id])}}" method="POST" class="form-control my-3" enctype="multipart/form-data">
    @csrf
    ...
    <div class="form-group">
        ...
        value="{{old('descripcion' , $posts->descripcion)}}"
        >
        @error('descripcion')
    ...
    <div class="form-group">
        ...
        <option value="{{$categoria->categoria_id}}"
            @selected(old('categoria_id' , $categoria->categoria_id) === $categoria->categoria_id)
    ...
    <div class="form-group">
        ...
        value="{{old('imagen_descripcion' , $posts->imagen_descripcion)}}"
        >
        @error('imagen_descripcion')
    ...
    <div class="form-group">
        ...
        @enderror>{{old('contenido' , $posts->contenido)}}</textarea>
        @error('contenido')
        <small id="contenido-help" class="form-text text-danger p-2">{{$message}}</small> 
        @enderror
    </div>
    <button type="submit" class="btn btn-primary my-3">Actualizar Post</button>
   </form>
@endsection
